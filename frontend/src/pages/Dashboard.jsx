  ...
  return (
    <div style={{ background: '#0a0a1a', minHeight: '100vh', color: 'white', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', padding: '1rem 2rem' }}>
        ...
        <button style={tabStyle('fly')} onClick={() => setActiveTab('fly')}>🚀 Fly Ship</button>
      </div>
      <div style={{ padding: '0 2rem 2rem' }}>
        {activeTab === 'map' && (
          <Galaxy3D planets={planets} routes={routes} routeResult={null} startId={null} endId={null} onPlanetClick={(p) => console.log('Clicked:', p.name)} />
        )}
        {activeTab === 'routes' && (
        ...
        {activeTab === 'fly' && planets.length > 0 && (
          <FlyMode planets={planets} />
        )}
	{activeTab === 'fly' && planets.length === 0 && (
          <div style={{ textAlign: 'center', padding: '3rem', color: '#888' }}>Loading planets...</div>
        )}
      </div>
    </div>
  )
